...
class CryptoViewModel: ViewModel() {
    ...
    private val map = ArrayMap<String, MutableLiveData<CryptoTrade>>()
    private val _liveMap = MutableLiveData<ArrayMap<String, MutableLiveData<CryptoTrade>>>()
    val liveMap: LiveData<ArrayMap<String, MutableLiveData<CryptoTrade>>> = _liveMap
            ...
            tradeList.forEach { cryptoTrade ->
                when{
                    map.contains(cryptoTrade.pair) -> {
                        map[cryptoTrade.pair]?.postValue(cryptoTrade)
                    }
                    else -> {
                        map[cryptoTrade.pair] = MutableLiveData(cryptoTrade)
    ...
    fun observe(symbol: String): LiveData<CryptoTrade>?{
